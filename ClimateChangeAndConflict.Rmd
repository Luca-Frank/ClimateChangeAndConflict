...
# dis<- na.omit(dis)
dis <- dis %>%
  mutate("country" = countrycode(dis$sov_a3, "genc3c", "iso3c")) 
...
```{r}
#filter for africa
dis <- left_join(dis,africa_code, by=c("sov_a3"="ISO"))
...
CPI <- CPI %>%
  dplyr::select(ISO,Y2000,Y2001,Y2002,Y2003,Y2004,Y2005,Y2006,Y2007,Y2008,Y2009,Y2010,Y2011,Y2012,Y2013,Y2014,Y2015,Y2016,Y2017,Y2018,Y2019,Y2020)
...
#types flood, extreme temperature, drought 
# floods over 10 and 5 years as control variable for meditation
flood10 <- cata %>%
  filter(disastertype == "flood",year>=2005, year<=2015)


drought20 <- cata %>%
  filter(disastertype== "drought", year>=1995, year<=2015)
drought30 <- cata %>%
  filter(disastertype== "drought", year>=1985, year<=2015)
...
dis$cata <- lengths(st_intersects(dis,cata))

dis$flood10 <- lengths(st_intersects(dis,flood10))

dis$drought20 <- lengths(st_intersects(dis,drought20))
dis$drought30 <- lengths(st_intersects(dis,drought30))
...
dis <- left_join(dis,vdem_15, by=c("sov_a3"="ISO"))
...
vdem_war <- vdem %>%
  ...
  distinct() %>%
  mutate(civil = 1,ISO= countrycode(country_name,"country.name","iso3c")) 


dis <- left_join(dis,vdem_war, by=c("sov_a3"="ISO"))
...
dis <- left_join(dis,fsi,by=c("sov_a3"="ISO"))
